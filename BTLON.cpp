...
void timkiem(GH &source);
void menu(FILE *file,char *path,GH source);
int main(){
    GH danhsach;
    FILE *file;
    char *path ="gioHang.dat";
...
void menu(FILE *file,char *path,GH source){
    ...
    do{
        ...
        printf("\n5. tim kiem ten khach hang.");
        printf("\n6. In ra file nhi phan.");
        printf("\n7. Thoat chuong trinh (ESC).");
        printf("\n---------------------------------------");
        printf("\n=>Lua chon cua ban la: ");
        chon = getch();
        switch (chon)
...
void inThongTinGioHang(GH output){
	...
	for(int i = 0; i<output.soluong_sp;i++){
		printf("\n%d||%s\t\t||%s\t\t||%d\t\t||%d\t\t||%d",i+1,output.ds_hang[i].msp,output.ds_hang[i].tensp,output.ds_hang[i].gia,output.ds_hang[i].sl,output.ds_hang[i].tongtien);
	}
	printf("\n---------------------------------------------------------\n");
}
void tinhTien(SP &input){
...
void timkiem(GH &source){
	...
	for(int i=0;i<n;i++){
		if(strcmp(ten,source.tenKhach)==0){
			inTheoDS(source);
		}break;
	}
}
void inRafile(FILE *file, char *path, GH source){
	...
	fprintf(file,"Ten khach hang: %s", source.tenKhach);
	fprintf(file,"\nDia chi khach hang: %s", source.diaChi);
	fprintf(file,"\nSo luong san pham mua: %d", source.soluong_sp);
	fprintf(file,"\nSTT||ID\t\t||Ten\t\t||Gia\t\t||SL\t\t||Tong");
	for(int i = 0; i < source.soluong_sp; i++){
		fprintf(file,"\n%d||%s\t\t||%s\t\t||%d\t\t||%d\t\t||%d",i+1,source.ds_hang[i].msp,source.ds_hang[i].tensp,source.ds_hang[i].gia,source.ds_hang[i].sl,source.ds_hang[i].tongtien);
	}
	fclose(file);
}
